...
import type { DeletionHistoryItem } from '@mypass360/types'
import { fetchDeletionHistory } from '../admin.service'
import { formatDate } from '../admin.utils'
import { SearchableEventSelect } from './SearchableEventSelect'

export function DeletionHistorySection() {
  const [selectedEventId, setSelectedEventId] = useState('')
  const [items, setItems] = useState<DeletionHistoryItem[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [selectedReasonModal, setSelectedReasonModal] = useState<{ title: string; reason: string; rejectionReason?: string | null } | null>(null)

  const loadHistory = useCallback(async (eventId?: string) => {
    setIsLoading(true)
    setError(null)
    try {
      const res = await fetchDeletionHistory({
        eventId: eventId || undefined,
        limit: eventId ? 100 : 10,
      })
      setItems(res.items)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro ao carregar histórico de exclusões.')
    } finally {
      setIsLoading(false)
    }
  }, [])
    ...
    <div style={{ display: 'grid', gap: '1.25rem' }}>
        ...
        {isLoading ? (
          ...
          <div style={{ padding: '4rem 1.5rem', textAlign: 'center', color: '#64748b' }}>
            ...
            <p style={{ margin: '4px 0 0', fontSize: '0.82rem', color: '#94a3b8' }}>
              As solicitações de exclusão processadas pelos administradores aparecerão aqui.
            </p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569' }}>
                  ...
                  <th style={{ padding: '0.85rem 1rem', fontWeight: 700 }}>Motivo do Pedido</th>
              ...
              <tbody>
                {items.map((item) => {
                  const isApproved = item.deletionStatus === 'approved'
                  return (
                    <tr
                      key={item.id}
                      style={{
                    ...
                    >
                      ...
                      <td style={{ padding: '0.85rem 1rem', color: '#334155', maxWidth: '240px' }}>
                        <p style={{ margin: 0, fontSize: '0.82rem', wordBreak: 'break-word', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          &quot;{item.deletionReason ?? 'Não informado'}&quot;
                        </p>
                      </td>

                      <td style={{ padding: '0.85rem 1rem', color: '#64748b', whiteSpace: 'nowrap' }}>
                        {formatDate(item.deletionRequestedAt ?? null)}
                      </td>

                      <td style={{ padding: '0.85rem 1rem', color: '#64748b', whiteSpace: 'nowrap' }}>
                        {formatDate(item.deletionReviewedAt ?? null)}
                      </td>
                      ...
                      <td style={{ padding: '0.85rem 1rem', whiteSpace: 'nowrap' }}>
                        ...
                        >
                          {isApproved ? '✓ Exclusão Aprovada' : '🛡️ Exclusão Mantida'}
                        </span>
                      </td>
                        ...
                        <button
                          ...
                          onClick={() =>
                            setSelectedReasonModal({
                              title: item.title,
                              reason: item.deletionReason ?? 'Não informado',
                              rejectionReason: item.deletionRejectionReason,
                            })
                          }
                          style={{
      ...
      {selectedReasonModal && (
        ...
        >
          ...
          >
            <h3 style={{ margin: '0 0 0.5rem', fontSize: '1.1rem', color: '#0f172a', fontWeight: 800 }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase', display: 'block', marginBottom: '0.3rem' }}>
                Motivo Enviado pelo Organizador:
              </span>
              <div style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '0.75rem', fontSize: '0.85rem', color: '#334155' }}>
                &quot;{selectedReasonModal.reason}&quot;
              </div>
            </div>

            {selectedReasonModal.rejectionReason && (
              <div style={{ marginBottom: '1rem' }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#991b1b', textTransform: 'uppercase', display: 'block', marginBottom: '0.3rem' }}>
                  Justificativa de Reprovação pelo Administrador:
                </span>
                <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '0.75rem', fontSize: '0.85rem', color: '#7f1d1d' }}>
                  &quot;{selectedReasonModal.rejectionReason}&quot;
                </div>
              </div>
            )}
